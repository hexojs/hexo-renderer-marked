...
Renderer.prototype.link = function(href, title, text) {
  const { options } = this;
  const { external_link, nofollow } = options;
  ...
  if (nofollow) {
    if (nofollow.enable && isExternal(href, options.config, nofollow.exclude)) {
      if (out.includes(noopener)) {
        out = out.replace(noopener, nofollowTag);
      } else {
        out += ' ' + nofollowTag;
      }
    }
