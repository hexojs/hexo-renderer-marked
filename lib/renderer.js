...
Renderer.prototype.listitem = function(text) {
  ...
  if (/^\s*(<p>\s*)?\[[x ]\]\s*/i.test(text)) {
    text = text.replace(/^\s*(<p>\s*)?\[ \]\s*/i, '<li style="list-style: none"><input type="checkbox"></input></li>').replace(/^\s*(<p>\s*)?\[x\]\s*/i, '<li style="list-style: none"><input type="checkbox" checked></input></li>');
    result = text;
  } else {
    result = '<li>' + text + '</li>\n';
  }
