...
function addBaseLink(href, baseLink){
  ...
  if(baseLink == undefined) return href;
...
module.exports = function(data, options){
  ...
  if(!!this.config.post_asset_folder){
    ...
    if(this.config.permalink != ':title/'){
      
      this.log.info('marked permalink must be :title/ (current : ' + this.config.permalink + ') for relative post_asset_folder');
      
    }
    // this.log.info('marked this: ', this);
    
    var postPath = path.join(this.source.context.source_dir, '_posts');
    var pathDir = path.dirname(data.path);
