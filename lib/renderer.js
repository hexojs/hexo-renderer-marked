...
const { encodeURL, escapeHTML, slugize, stripHTML, url_for, isExternalLink } = require('hexo-util');
...
class Tokenizer extends MarkedTokenizer {
  ...
  inlineText(src, inRawBlock, smartypants) {
    const { quotes, smartypants: smartypantsCfg } = this.options;
    const validQuotes = quotes && quotes.length === 4;
    if (smartypantsCfg && validQuotes) {
      ...
      if (match) {
        return {
          ...
          text: match[0]
            ...
            // opening doubles
            .replace(/(^|[-\u2014/([{\u2018\s])"/g, '$1' + quotes[0])
            // closing doubles
            .replace(/"/g, quotes[1])
            // opening singles
            .replace(/(^|[-\u2014/([{"\s])'/g, '$1' + quotes[2])
    ...
    if (cap) {
      ...
      if (inRawBlock) {
        ...
        text = escapeHTML(smartypantsCfg ? smartypants(cap[0]) : cap[0]);
      }
      return {
        type: 'text',
        raw: cap[0],
